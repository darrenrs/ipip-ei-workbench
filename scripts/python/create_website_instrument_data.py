#!/usr/bin/env python3
"""
Create frontend-ready JSON instrument data files under:
  website/public/instruments/

Outputs:
- one generated JSON file per instrument
"""
...
def write_output_files(output_dir: Path, generated: dict[str, dict[str, Any]]) -> None:
    ...
    for config in INSTRUMENT_CONFIGS:
        ...
        file_path.write_text(
            json.dumps(generated[slug], indent=2, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Output directory. Defaults to website/public/instruments",
    )
    args = parser.parse_args()

    repo_root = Path(__file__).resolve().parents[2]
    output_dir = args.output_dir or (
        repo_root / "website" / "public" / "instruments"
    )
